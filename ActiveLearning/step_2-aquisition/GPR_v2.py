...
class GaussianProcessRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        try:
            ...
            self._K_inv = scipy.linalg.inv(K)
        ...
        self.alpha_ = self._K_inv.dot(self.y_train_)  # Line 3
        return self
    ...
    def log_marginal_likelihood(self, theta=None, eval_gradient=False):
        ...
        # Change lines below if we plan to actually use the log-marginal likihood
        if theta is None:
            if eval_gradient:
                raise ValueError(
        ...
        #changed below, probably wrong
        log_likelihood_dims -= np.log(np.diag(K_inv)).sum()
